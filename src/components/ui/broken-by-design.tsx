...
/*                                                                    */
/*  Izakaya Zen customization: the shard's visible texture is one of  */
/*  the restaurant's own dish photos (foodImages prop) instead of a   */
/*  plain glass render — the mask (the shard's jagged silhouette)     */
/*  still comes from the original glass atlas, only what's shown      */
/*  *through* that silhouette changed. So it still reads as broken    */
/*  glass, but each piece now frames a bite of food instead of air.   */
/* ------------------------------------------------------------------ */

export interface BrokenByDesignProps {
  /** Base URL where the shard mask/sound PNGs are hosted (no trailing slash). */
  assetsBase?: string
  title?: string
  height?: string
  ...
  interactive?: boolean
  className?: string
  /** Dish photos shown through the glass shards, cycled round-robin per piece. */
  foodImages?: string[]
}
...
/* One sprite per breakpoint instead of 16 separate PNGs — cuts the
   pane from ~16 HTTP requests to 1. Used ONLY as the alpha mask that
   gives each shard its jagged broken-glass silhouette; the visible
   texture inside that silhouette comes from foodImages instead. */
const ATLAS = {
...
/* Classic responsive-sprite math: background-size/position expressed
   ...
   without knowing actual pixel dimensions up front. Used for the mask. */
...
const DEFAULT_FOOD_IMAGES = [
  'https://lh3.googleusercontent.com/aida-public/AB6AXuDw0yTfD24pJsmnBqIGFZxwyo1yKRVqaLDEvGYirBBL15aF5G9YTt4CzDI6Rzq2kvymL8yBzwPg-lCHb9PWElvD25viRK9vkzclY11wlD1yUHdhOhnfrFehv-boDXM6cEKiZnLCzNxmGWy0hOKY2DRVF95IaBchOVNTdtQgVbQHREFngsL5jjud-5I5kTW2p9Mw8h243aFuOCS3JwFqE7Lx2N2Dm-0IQGA8Ia0nR__fvj4_zQpwLd9g',
  'https://lh3.googleusercontent.com/aida-public/AB6AXuBwyfxEVbKthIhYjG35g7BXAVXs0nKANkNjABZ9sRM-elLQ6UGvAAoh4R3ZJVIdwi_VLD6HK1q7VDkcsC3CyxtH5VYdLV52SNKmAbZhfGo1N6vP46uN0AzDE0RwGAtzfTcidjWxwOO6n3Kzj8LVs61wP26EXMRE4pbfPreO_GgVYHtaOq_qkjuIE7uezOIqMu_eyFlDsL7QjiqMXhB-nyh6-iDKPpqXQ_o2kufR3sudNBN4mWGTxKlm',
  'https://lh3.googleusercontent.com/aida-public/AB6AXuDXlvbY8syfGaV_rVsliz0UX5CiuJScXxtYS7ODWyhelDqB3NVr71Yk-SRhrD9apj1tds6L0SDEB6pKHVSKupUWj15_o9edgHBhPulfe0Td9AYoe5V-Q6Rr0Pqun72XZIZkbAchCdTiF_lFZZuPxfc-eAgdsDjfFeRKZHBELK46bOSh-DPAdCnPe8SWd8mcCjYizLJvlN20uVDMMWuPIjGK1egj00hVz_0TlyA9Zy8oa49iA_YDWDT5',
  'https://lh3.googleusercontent.com/aida-public/AB6AXuC6ZZcgJSL5wnmp3m7Q4h9nxRajGkGLMJcq_8JwmBMHdaD_hRMa5HofrZjO32eUHfg30WX0kqlGi7tD_FcIPzjD6y8n1YKA_jBOepOBu0n8t7nF1pMlByt76i0v-zYuj9S9OHdTXsTdyU6IzeMNFuJ2L1yLx_3pbSjdAWofxrAyEKqUunCLoR_ujSAcr3cqN2EV-kw6Dk7RfyZrTSR-Bk1RbAEiyWiFShp3RbTH70u-SvKPQwOs1EI7',
  'https://lh3.googleusercontent.com/aida-public/AB6AXuDntqiwqh9GWfwMPB1U15T3ENTdqrtl0IGiUSwMeK8wjhmeheZHimifxPus14AWq57fYwO-K3EpsdrRaYanGFiKF74axTWHfOZS0ZZHx17_DOb-p2x75iUCtrHYOZJxpCHLZ-YTthYBkCHRPxBbPH8kB32MbQmPzezAw0RYYbmNQk5gHzlQH1DeYZWPcQ5uBlTZr4ZzIqYURQJn7ScX5zoDFFyutio8zRfRMHCEyb8VaQDqDUjpP8Qe',
]
...
export default function BrokenByDesign({
  ...
  interactive = true,
  className = '',
  foodImages = DEFAULT_FOOD_IMAGES,
}: BrokenByDesignProps) {
  ...
  /* Gate the entrance on the mask sprite actually being decoded — a single
  ...
  return (
    ...
    >
      ...
      {/* Nothing below renders until the mask sprite is actually decoded —
      ...
      <div className="bbd2-pane" aria-hidden="true">
        {pieces.map((p, i) => {
          const j = jitters[i]
          const maskUrl = `${assetsBase}/${ATLAS[setKey].url}`
          const dishUrl = foodImages[i % foodImages.length]
